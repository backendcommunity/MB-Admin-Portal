...
router.get('/', async (req, res) => {
    const { q, role, active, page = '1', pageSize, limit = '20', sort = 'id', order = 'asc' } = req.query;
    const where = q
...
router.post('/', async (req, res) => {
    ...
    try {
        const user = await db_1.prisma.user.create({ data: { email, name, role } });
...
router.patch('/:id', async (req, res) => {
    ...
    try {
        if (action === 'suspend') {
            ...
            const user = await db_1.prisma.user.update({ where: { id }, data: { active } });
